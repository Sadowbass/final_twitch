...
public class Handler_one extends TextWebSocketHandler {
				...
				for(WebSocketSession s:list) {
					...
					if(compare!=null && mid!=null) {
						...
						if(mid.equals(compare)) {
							if(mid.equals(censorship)) {/*스트리머 재 접속시*/
								reduplication=false; /*입장 거부*/

								/* 재접속 스트리머에게 보낼 로그인한 유저 목록 */
								List<WebSocketSession>list1=chatRoom.get(censorship);
								/* 채팅방 입장한 재접속 스트리머에게 채팅방 유저 목록 json으로 변환해서 */
								List<String> nameList=new ArrayList<String>();
								for(WebSocketSession s1:list1) {
									nameList.add((String)s1.getAttributes().get("session_id"));
								}
								...
								if(chatRoom.get(censorship)!=null) {
									...
									/*채팅방 사람들에게 뿌려줌*/
									session.sendMessage(new TextMessage(jsonTxt4));
								}

								chatRoom.get(censorship).add(session);
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		/*(5)리얼 방종*/
		if(ele.getAsJsonObject().get("realClose")!=null) {
